...
if __name__ == '__main__':
    ...
    for frame in range(7,15):
        ...
        plt.savefig('messi/plot/fx/frame{0:02d}.png'.format(frame), dpi=100)
        plt.clf()

        imgplot = plt.imshow(flowy,cmap = plt.cm.bwr , norm = norm)
        plt.colorbar(imgplot)
        plt.savefig('messi/plot/fy/frame{0:02d}.png'.format(frame), dpi=100)
        plt.clf()
        ...
        cv2.imwrite('./messi/messigray{}.png'.format(frame),imgf)
